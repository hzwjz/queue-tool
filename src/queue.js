...
export default class Queue {
    ...
    add(task){
        if(!(task instanceof Task)){
            return Promise.reject('type error: the queue\'s task should be instance of Task Class');
        }
    ...
    _checkRun(){
        ...
        for(let i = 0; i < this._taskList.length; i++){
            if(this._taskList[i].getStatus() == TaskStatus.TASK_STATUS_NOTSTART){  
                ...
                // not delete
                // if(this._taskList[i].getStatus() == TaskStatus.TASK_STATUS_END || 
